...
- `gov.nasa.jpl.fprime.yamcs.filetransfer.FprimeFilePacketService` — a YAMCS `FileTransferService` implementing `Fw::FilePacket` file uplink and downlink. Downlinked files are stored in a YAMCS bucket (and optionally mirrored to a local directory via `downlinkMirrorDir`, default `/tmp/fprime-downlink`).
