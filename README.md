...
- `gov.nasa.jpl.fprime.yamcs.filetransfer.CfdpSpacePacketBridge` — shuttles CFDP PDUs between the F´ space-packet pipelines and the `cfdp_in`/`cfdp_out` streams consumed by YAMCS's **built-in** CFDP implementation: on downlink it unwraps space packets on the CFDP APID (stripping the F´ `FW_PACKET_FILE` descriptor), on uplink it wraps outgoing PDUs and routes them through a configured TC link. All CFDP protocol logic lives in stock YAMCS.
- `gov.nasa.jpl.fprime.yamcs.filetransfer.FprimeCfdpService` — YAMCS's native `org.yamcs.cfdp.CfdpService` extended with the F´ command side only: download requests synthesize a configurable F´ downlink command (F´ has no CFDP proxy-put), and remote file listing synthesizes `FileManager.ListDirectory` and assembles results from F´ events. See the commented example in `yamcs.fprime-project.yaml`.

The file transfer services route uplink through the YAMCS data link named by the `uplinkLink` option. Any `TcDataLink` is accepted: a CCSDS TC frame virtual channel (e.g. `UDP_TC_OUT.vc1`, the default TM/TC pipeline) **or** a raw space-packet-only link such as `org.yamcs.tctm.UdpTcDataLink` — the services are not tied to the TC framing pipeline. See the `FprimeFilePacketService`, `CfdpSpacePacketBridge`, and `FprimeCfdpService` class Javadoc for the per-service option lists, and `src/fprime_yamcs/yamcs/src/main/yamcs/etc/yamcs.fprime-project.yaml` for a working configuration.
