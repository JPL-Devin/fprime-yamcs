...
def check_pip_only_environment():
    """Confirm the pip-provided runtime is used and no build tooling is present"""
    assert shutil.which("mvn") is None, "mvn must not be installed for this smoke test"
    assert shutil.which("java") is None, "java must not be on the PATH for this smoke test"
    from fprime_yamcs.java import find_java
    from fprime_yamcs_runtime import JAVA
    java = find_java()
    assert java == Path(JAVA), f"Expected the pip-provided runtime {JAVA}, resolved {java}"
...
def start_yamcs(java: Path, data_dir: Path) -> subprocess.Popen:
    ...
    from fprime_yamcs.java import build_classpath
    package_dir = Path(fprime_yamcs.java.__file__).resolve().parent
    config_dir = package_dir / "yamcs" / "src" / "main" / "yamcs"
    command = [
        str(java), "-Djava.util.logging.manager=org.yamcs.logging.YamcsLogManager",
        "-cp", build_classpath([]), "org.yamcs.YamcsServer",
        "--etc-dir", str(config_dir / "etc"), "--data-dir", str(data_dir),
    ]
...
def await_yamcs(process: subprocess.Popen) -> dict:
    ...
    while time.time() < deadline:
        ...
        try:
            with urllib.request.urlopen(f"{YAMCS_URL}/api", timeout=5) as response:
...
def main() -> int:
    java = check_pip_only_environment()
    with tempfile.TemporaryDirectory() as data_dir:
        ...
        try:
            ...
            assert any("fprime" in plugin for plugin in plugins), \
                f"F Prime plugin not loaded. Plugins: {plugins}"
            with urllib.request.urlopen(YAMCS_URL, timeout=5) as response:
                assert response.status == 200, f"yamcs-web returned {response.status}"
